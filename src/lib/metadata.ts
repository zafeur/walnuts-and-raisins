...
export const isPublic =
  !business.isDraft &&
  !new URL(siteUrl).hostname.match(/^(localhost|127\.0\.0\.1)$/);
export function pageMetadata(locale: Locale, contact = false): Metadata {
  ...
  return {
    ...
    title: `${business.name[locale]} | ${contact ? copy.contactTitle : copy.title}`,
    ...
    icons: { icon: "/images/zarchin-logo-transparent.png" },
  };
}
